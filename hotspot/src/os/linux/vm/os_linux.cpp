...
static void *java_start(Thread *thread) {
  ...
  int pid = os::current_process_id(); // 得到pid
  alloca(((pid ^ counter++) & 7) * 128); // alloca分配的内存会自动释放
  ...
  // handshaking with parent thread
  {
    ...
    // notify parent thread
    osthread->set_state(INITIALIZED); //1、设置初始化状态
    sync->notify_all();
...
bool os::create_thread(Thread* thread, ThreadType thr_type, size_t stack_size) {
  ...
  if (os::Linux::supports_variable_stack_size()) {
    ...
    if (stack_size == 0) {
      stack_size = os::Linux::default_stack_size(thr_type); // 得到默认分配的stack内存
