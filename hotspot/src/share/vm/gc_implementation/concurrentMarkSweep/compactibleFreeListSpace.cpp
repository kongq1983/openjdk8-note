...
CompactibleFreeListSpace::CompactibleFreeListSpace(BlockOffsetSharedArray* bs,
  ...
  _rescan_task_size(CardTableModRefBS::card_size_in_words * BitsPerWord * // //并行标记阶段一次rescan处理的内存块的大小
                    CMSRescanMultiple),
  _marking_task_size(CardTableModRefBS::card_size_in_words * BitsPerWord * //并行标记阶段一次mark 处理的内存块的大小
                    CMSConcMarkMultiple),
  _collector(NULL)
{
...
// Apply the given closure to each block in the space.
void CompactibleFreeListSpace::blk_iterate(BlkClosure* cl) { // TODO blk_iterate
  assert_lock_strong(freelistLock());
  HeapWord *cur, *limit;
  for (cur = bottom(), limit = end(); cur < limit;
       cur += cl->do_blk(cur));
...
void CompactibleFreeListSpace::oop_iterate(ExtendedOopClosure* cl) { // TODO oop_iterate
  assert_lock_strong(freelistLock());
  HeapWord *cur, *limit;
  size_t curSize;
  for (cur = bottom(), limit = end(); cur < limit;
...
void CompactibleFreeListSpace::object_iterate(ObjectClosure* blk) { // TODO object_iterate
  assert_lock_strong(freelistLock());
