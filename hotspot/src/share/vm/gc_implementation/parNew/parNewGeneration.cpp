    ...
    _gen(gen), _next_gen(old_gen), _term(term)
{
  ...
  // Initialize states.
  for (int i = 0; i < num_threads; ++i) { // 初始化多个线程
...
void ParNewGeneration::collect(bool   full,
  ...
  AdaptiveSizePolicy* size_policy = gch->gen_policy()->size_policy(); // TODO AdaptiveSizePolicy
  FlexibleWorkGang* workers = gch->workers();
  ...
  if (UseAdaptiveSizePolicy) { // TODO UseAdaptiveSizePolicy
