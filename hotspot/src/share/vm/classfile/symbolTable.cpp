...
oop StringTable::intern(Handle string_or_null, jchar* name,
  ...
  int index = the_table()->hash_to_index(hashValue); //根据哈希值计算出索引，和HashMap有点像
  oop found_string = the_table()->lookup(index, name, len, hashValue); // 在常量池寻找该字符串

  // Found
  if (found_string != NULL) { // 找到了
  ...
  oop added_or_found;
  {
    ...
    // Otherwise, add to symbol to table 将字面量添加到table
