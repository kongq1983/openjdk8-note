...
class EPollArrayWrapper {
    ...
    // Miscellaneous constants   sizeof(struct epoll_event)
    ...
    EPollArrayWrapper() throws IOException {
        ...
        epfd = epollCreate(); // 创建epoll池 int epfd = epoll_create(256);
    ...
    void initInterrupt(int fd0, int fd1) {
        outgoingInterruptFD = fd1; // 写
        incomingInterruptFD = fd0; // 读
        epollCtl(epfd, EPOLL_CTL_ADD, fd0, EPOLLIN); // 注册读事件
    }
    ...
    private void setUpdateEvents(int fd, byte events, boolean force) {
        if (fd < MAX_UPDATE_ARRAY_SIZE) {
            if ((eventsLow[fd] != KILLED) || force) { // 传过来force=true
                eventsLow[fd] = events;  //设置0   eventsLow[fd] = 0
            }
        } else {
