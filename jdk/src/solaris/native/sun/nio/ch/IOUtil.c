...
{
    int flags = fcntl(fd, F_GETFL); // 获取文件打开方式的标志，标志值含义与open调用一致
    int newflags = blocking ? (flags & ~O_NONBLOCK) : (flags | O_NONBLOCK);  // 非阻塞 传入的是blocking=false
...
Java_sun_nio_ch_IOUtil_makePipe(JNIEnv *env, jobject this, jboolean blocking)
{
    ...
    return ((jlong) fd[0] << 32) | (jlong) fd[1]; // fd[0] 读 往左移动32位 | 写32位
}
